...
def construct_milp_constraint(ts, type_num, poset, pruned_subgraph, element2edge, element_component_clause_literal_node,
    ...
    m = Model()
    # create variables
    x_vars, t_vars, c_vars, t_edge_vars, b_element_vars, b_minimal_element_vars, b_immediate_element_vars, \
    ...
    for element in poset:
        ...
        edge_constraints(m, ts, x_vars, t_vars, c_vars, t_edge_vars, b_element_vars, b_minimal_element_vars, element,
                         ...
                         M, epsilon, buchi, strict_larger_element, incomparable_element, minimal_element,
    ...
    activation_first(m, ts, x_vars, t_vars, c_vars, b_element_vars,
                     ...
                     maximal_element, b_maximal_element_vars, init_type_robot_node)
...
def create_variables(m, ts, poset, pruned_subgraph, element2edge, type_num, minimal_element, maximal_element):
    ...
    return x_vars, t_vars, c_vars, t_edge_vars, b_element_vars, b_minimal_element_vars, b_immediate_element_vars, \
           b_maximal_element_vars
...
def one_clause_true(m, c_vars, b_minimal_element_vars, element, component, label, strict_larger_element,
...
def edge_constraints(m, ts, x_vars, t_vars, c_vars, t_edge_vars, b_element_vars, b_minimal_element_vars, element,
    ...
    if edge_label != '1':
        ...
        one_clause_true(m, c_vars, b_minimal_element_vars, element, 1, edge_label, strict_larger_element,
                        incomparable_element,
                        buchi, minimal_element)
    ...
    if element in minimal_element:
        ...
        for i in clause_nodes:
            m.addConstr(quicksum(t_vars[(clause_nodes[0], k, 1)]
                                 for k in
                                 range(type_num[ts.nodes[clause_nodes[0]]['location_type_component_element'][1]])) ==
                        quicksum(t_vars[(i, k, 1)] for k in
                                 range(type_num[ts.nodes[i]['location_type_component_element'][1]])))

        # one of the extra literals in the last subtasks must be satisfied -- (31)
        z = len(minimal_element) - 1
        m.addConstr(1 + M * (quicksum(b_element_vars[(element, another_element)]
                                      for another_element in minimal_element if another_element != element) - z)
                    <= b_minimal_element_vars[element])
        m.addConstr(b_minimal_element_vars[element] <= 1 + M * (z - quicksum(b_element_vars[(element, another_element)]
...
def self_loop_constraints(m, ts, x_vars, t_vars, c_vars, t_edge_vars, b_element_vars, b_minimal_element_vars, element,
                          self_loop_label, strict_larger_element, incomparable_element, poset_relation,
                          element_component_clause_literal_node,
                          type_num, M, buchi):
    # one and only one clause is true, -- eq (9)
    one_clause_true(m, c_vars, b_minimal_element_vars, element, 0, self_loop_label, strict_larger_element,
                    incomparable_element, buchi, [])
...
def activate_next(m, ts, x_vars, t_vars, c_vars, t_edge_vars, b_element_vars, b_immediate_element_vars,
    ...
    # subtask can be the last one -- eq (20)
        ...
        m.addConstr(quicksum(b_immediate_element_vars[(element, another_element)]
                             for another_element in incomparable_element[element]) <= 1)
        m.addConstr(z - quicksum(b_element_vars[(element, another_element)]
                                 for another_element in incomparable_element[element]) -
                    M * quicksum(b_immediate_element_vars[(element, another_element)]
                                 for another_element in incomparable_element[element]) <= 0)

        m.addConstr(quicksum(b_immediate_element_vars[(element, another_element)]
                             for another_element in incomparable_element[element]) - M *
                    (z - quicksum(b_element_vars[(element, another_element)]
                                  for another_element in incomparable_element[element])) <= 0)
    m.update()
    ...
    if strict_larger_element[element]:
        ...
        m.addConstr(quicksum(b_element_vars[(element, another_element)]
                             for another_element in incomparable_element[element]) -
                    M * quicksum(b_immediate_element_vars[(another_element, element)]
                                 for another_element in incomparable_element[element]) <= 0)
        m.addConstr(quicksum(b_immediate_element_vars[(another_element, element)]
                             for another_element in incomparable_element[element]) -
    ...
    for another_element in strict_smaller + incomparable_element[element]:
        m.addConstr(t_edge_vars[element] + 1 <= t_edge_vars[another_element] +
                    M * (1 - b_immediate_element_vars[(element, another_element)]))
